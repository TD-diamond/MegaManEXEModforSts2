...
public class ElecStylePower():PowerAbstracts(PowerType.Debuff,PowerStackType.Counter)
{
    public override string CustomPackedIconPath => $"{customIconRoot}/elec_style_power.png";
    public override string CustomBigIconPath => $"{customIconRoot}/elec_style_power.png";
    ...
    {
        if (target.HasPower<ElecStylePower>())
            await PowerCmd.Remove<ElecStylePower>(target);
    }
    ...
    public override async Task AfterPowerAmountChanged(PowerModel power, decimal amount, Creature applier, CardModel cardSource)
    {
        ...
        {
            isRestoring = true;
            await PowerCmd.SetAmount<ElecStylePower>(Owner, lockedAmount, Owner, null);
            isRestoring = false;
        }
    }
