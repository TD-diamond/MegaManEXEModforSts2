...
public class GrassStylePower():PowerAbstracts(PowerType.Debuff,PowerStackType.Counter)
{
    public override string CustomPackedIconPath => $"{customIconRoot}/grass_style_power.png";
    public override string CustomBigIconPath => $"{customIconRoot}/grass_style_power.png";
    ...
    {
        if (target.HasPower<GrassStylePower>())
            await PowerCmd.Remove<GrassStylePower>(target);
    }
    ...
    public override async Task AfterPowerAmountChanged(PowerModel power, decimal amount, Creature applier, CardModel cardSource)
    {
        ...
        {
            isRestoring = true;
            await PowerCmd.SetAmount<GrassStylePower>(Owner, lockedAmount, Owner, null);
            isRestoring = false;
        }
    }
