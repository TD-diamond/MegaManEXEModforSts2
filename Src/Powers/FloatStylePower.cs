...
public class FloatStylePower():PowerAbstracts(PowerType.Debuff,PowerStackType.Counter)
{
    ...
    {
        if (target.HasPower<FloatStylePower>())
            await PowerCmd.Remove<FloatStylePower>(target);
    }
    ...
    public override async Task AfterPowerAmountChanged(PowerModel power, decimal amount, Creature applier, CardModel cardSource)
    {
        ...
        {
            isRestoring = true;
            await PowerCmd.SetAmount<FloatStylePower>(Owner, lockedAmount, Owner, null);
            isRestoring = false;
        }
    }
